...
---
## 1. 문제를 푸는 시점에 컴파일러 동작 방식에 대해 어디까지 알고 있는가?
문제를 푸는 시점에서 사실 컴파일러가 무엇인지만 알았지 동작 방식까지는 전혀 모르는 상태였습니다.  
그래서 많이 찾아보고 학습하여 찾은 내용을 아래와 같습니다. 
...
## 2. 제공된 자료 중에서 어떤 자료를 학습하고 이해했는가?
제공된 자료들의 내용을 살펴보자면 아래와 같이 정리할 수 있을 것 같습니다. 
제공된 자료들도 분명 내용이 정말 좋았지만 이것만으로 공부하기엔 저에게 난이도가 너무높아서 게시글의 댓글들을 보며 하나하나 차근차근 짚어서 공부했습니다. 
댓글에 다른분들이 작업하며 '이 자료 좋았어요', '저 자료 좋았어요' 하는 것들을 모아서 보고 그 내용에 해당하는 자료들을 다시 한 번 보는 식으로 학습하였습니다. 
...
## 4. 이번 미션을 통해 학습한 것과 과정에서 배운 것들에 대한 설명 
배운 점 요약
...
뿐만 아니라 저는 이 게시글 보고 했을 때 어떤 부분이 좋았어요. 이 영상에서 이런 부분 설명해줘요. 라고 댓글로 올려주신 자료들을 보고 제공된 자료를 보는식으로 하여 단계별로 학습을 하였습니다. 

이번 리서치를 통해 AST의 실질적인 구성 방식, 언어별 차이, 트리 구조 구현 방식 등을 이해할 수 있었습니다.   
특히 AST 구조를 "실제 코드"로 표현하면서 추상적 개념이 어떻게 코드와 연결되는지 실감할 수 있었습니다.   
앞으로 더 복잡한 문장이나 함수 선언, 조건문 등을 포함한 AST 확장 설계로 발전해 나갈 계획입니다.   
아직 모르는 게 많지만 계속해서 학습하여 활용할 수 있도록 노력하겠습니다. 
